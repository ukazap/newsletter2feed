module AtomFeedBuildable
  XML_ENCODING = "UTF-8"
  ATOM_NAMESPACE = "http://www.w3.org/2005/Atom"

  def to_atom_xml
    builder = Nokogiri::XML::Builder.new(encoding: XML_ENCODING) do |xml|
  ...
  private
    def build_feed_metadata(xml)
      ...
      if icon.present?
        xml.icon { xml.text(icon) }
      end
    end

    def build_entries(xml)
      feed_entries.order(created_at: :desc).each do |entry|
    ...
    def feed_urn
      "urn:newsletter2feed:feed:#{public_id}"
    end
    ...
    def last_updated
      feed_entries.maximum(:updated_at) || updated_at
    end
